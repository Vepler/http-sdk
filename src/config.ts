...
const defaultConfig = {
  production: {
    propertyHost:
      process.env.SVC_PROPERTY_HOST || 'https://api2.propbar.co.uk/property',
    areaReferenceHost:
      process.env.AREA_REFERENCE_HOST ||
    ...
    schoolsHost:
      process.env.SCHOOLS_HOST || 'https://api2.propbar.co.uk/schools',
    planningRegisterHost:
      process.env.PLANNING_REGISTER_HOST ||
    ...
    propertyPredictorHost:
      process.env.PROPERTY_PREDICTOR_HOST ||
      'https://api2.propbar.co.uk/linked-avm',
    locatorHost:
      process.env.SVC_LOCATIONS_HOST || 'https://api2.propbar.co.uk/locator',
    councilRegisterHost:
      process.env.SVC_COUNCIL_TAX_HOST ||
...
export const initializeSDK = (
  config: SDKConfig = defaultConfig.production,
  env: 'production' | 'development' = 'production'
) => {
  // Prevent re-initialization
  if (isInitialized) {
    console.warn(
      'SDK has already been initialized. Call reset() first if you want to re-initialize.'
    );
    return;
  }
  ...
  try {
    ...
    apiInstances['planning-register'] = ApiService.create({
      ...commonConfig,
      host:
        config.planningRegisterHost || environmentConfig.planningRegisterHost,
    });
    ...
    apiInstances['property-predictor'] = ApiService.create({
      ...commonConfig,
      host:
        config.propertyPredictorHost || environmentConfig.propertyPredictorHost,
    });
...
export const getApiInstance = (service: string): any => {
  ...
  if (!apiInstance) {
    throw new Error(
      `Service '${service}' does not exist. Available services are: ${Object.keys(apiInstances).join(', ')}`
    );
  }
